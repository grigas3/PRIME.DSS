    ...
    public class DSSRunner : IDSSRunner
    {
        ...
        private async Task<IEnumerable<DSSValue>> GetInputValues(string patientId, DSSConfig config)
        {
            ...
            foreach (var c in config.Input)
            {
                ...
                {
                    throw new NotSupportedException($"Source type {c.Source} not supported from PRIME DSS");
                }
            }
                ...
                try
                {
                    var observations = await _dataProxy.Get<PDObservation>(10, 0,
                        String.Format(
                            ...
                            patientId, code,
                            (DateTime.Now.AddDays(-config.AggregationPeriodDays).ToUnixTimestampMilli()),
                    ...
                    foreach (var observation in config.Input.Where(e => e.Code == code))
                    {
                        ...
                        {
                            Name = observation.Name, Code = observation.Code,
                            Value = observations.Select(e => e.Value).DefaultIfEmpty(0).Average().ToString()
                        });
                    }
            ...
            foreach (var code in metaObservationCodes)
            {
                try
                {
                    ...
                    foreach (var metaObservation in config.Input.Where(e => e.Code == code))
                    {
                        ...
                        values.Add(new DSSValue()
                        {
                            Name = metaObservation.Name, Code = metaObservation.Code,
                            Value = aggrObservation.Select(e => e.Value).DefaultIfEmpty(0).Average().ToString()
                        });
                    }
                }
                catch (Exception ex)
